...
int main(void) {
    ...
    /** 공유 메모리에 저장된 구조체의 주소 값을 저장할 포인터 변수 */
    SHM_t *pSharedMemory = NULL;
    /** 공유 메모리에 접근 하기 위해서 식별자 가져오기 */
    shared_memory_id = openSharedMemory(5000);
    /** 공유 메모리에 저장된 값 읽어오기 */
    pSharedMemory = getSharedMemoryPointer(shared_memory_id);
    /** 공유 메모리에 데이터 쓰기 */
    pSharedMemory->count = 1234;
    /** 공유 메모리에 저장된 문자열 값 초기화 */
    memset(pSharedMemory->name, '\0', sizeof pSharedMemory->name);
    /** 공유 메모리에 문자열 데이터 쓰기 */
    strcpy(pSharedMemory->name, "shared memory");
    /** 대기하기 위한 함수 */
    getchar();
    /** 공유 메모리의 값 읽어오기 */
    printf("%d, %s \r\n", pSharedMemory->count, pSharedMemory->name);
    return 0;
}
